...
class Menu(synthmenu.Menu):
    ...
    def __init__(
        ...
        for i in range(self.lines):
            ...
            label_text.anchored_position = (
                self._width - BORDER_WIDTH - PADDING - SCROLLBAR_WIDTH,
                LINE_SIZE // 2,
            )
    ...
    def draw(self) -> None:
        ...
        if is_value_group:
            self._title_item[1].text = f"{item.title:s}: {item.current_item.title:s}"
        elif is_waveform:
            self._title_item[1].text = f"{item.title:s}: {item.label:s}"
        else:
        ...
        if not is_value:
            ...
            self._scrollbar_item.y = int(
                LINE_SIZE
                + (self._height - LINE_SIZE - self._scrollbar_item.height)
                * item.index
                / max(item.length - 1, 1)
            )
            ...
            if is_waveform_group:
                self._chart_group[1].x = min(
                    max(int(item.loop_start.value * self._width), 0), self._width - INDICATOR_STROKE
                )
                self._chart_group[2].x = min(
                    max(int(item.loop_end.value * self._width), 0), self._width - INDICATOR_STROKE
                )
            ...
            if isinstance(item, synthmenu.AREnvelope):
                ...
                if item.index == 0:  # attack_time
                    ...
                    pos = (
                        1 - (item.attack_time.relative_value + item.release_time.relative_value) / 3
                    ) / 2 + item.attack_time.relative_value / 3
                else:  # release_time
                ...
                if item.index == 0:  # attack_time
                    ...
                    pos = (
                        1
                        - (
                            item.attack_time.relative_value
                            + item.decay_time.relative_value
                            + item.release_time.relative_value
                        )
                        / 4
                    ) / 2 + (item.attack_time.relative_value + item.decay_time.relative_value) / 4
                else:  # release_time
    ...
    def _draw_item(self, index: int, item: synthmenu.Item, selected: bool = False) -> None:
        ...
        draw_item[1].pixel_shader[0] = (
            self.item_background_selected_color if selected else self.item_background_color
        )
