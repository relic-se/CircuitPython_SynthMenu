...
class Group(Item):
    ...
    def __init__(
        self, title: str | Callable[[Item], str], items: tuple[Item] = None, loop: bool = False
    ):
...
class Number(Item):
    ...
    def value(self) -> float | int:
        if self.smoothing:
            return (
                math.pow(self._value, self._smoothing) * (self._maximum - self._minimum)
                + self._minimum
            )
        else:
            return self._value
    ...
    def label(self) -> str:
        ...
        if type(value) is float:
            label = f"{{:.{self.decimals:d}f}}".format(value)
        else:
...
class Percentage(Number):
    ...
    @property
    def label(self) -> str:
        return f"{round(self.value * 100):d}%"
...
class Char(Number):
    def __init__(
        self, title: str | Callable[[Item], str], on_update: Callable[[str, Item], None] = None
    ):
        super().__init__(
            title,
            step=1,
...
class String(Group):
    def __init__(
        ...
        title: str | Callable[[Item], str],
        length: int = 16,
        on_update: Callable[[str, Item], None] = None,
    ):
        self._length = length
        super().__init__(
            title,
